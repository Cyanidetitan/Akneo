...
class Anime():
    ...
    async def event_handler_anime(event):
        if '/anime' == event.raw_text:
            await bot.send_message(
                ...
                file='https://c.tenor.com/cVB_s--gLXgAAAAd/akeno-ara-ara.gif'
            )
    ...
    async def event_handler_batch(event):
        ...
        try:
            ...
            if int(split_data[2]) - int(split_data[1]) > 24:
                await event.reply(
                    "Batch Download is capped at 24 episodes \nPlease download in batches of less than 24."
                )
    ...
    async def event_handler_batch(event):
        try:
            ...
            if int(split_data[2]) - int(split_data[1]) > 100:
                await event.reply(
                    "Batch Download is capped at 100 episodes \nPlease download in batches of less than 100."
                )
                return
            list_of_links = []
            await event.reply("Be Patient till i gather the data....")
            for i in range(int(split_data[1]), (int(split_data[2]) + 1)):
